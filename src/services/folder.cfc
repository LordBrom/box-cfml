...
component output="false" hint="Box service layer for folder objects." {
	...
	public folder function init(boxAPIHandler boxAPIHandler) output=false {
		variables.boxAPIHandler = arguments?.boxAPIHandler ?: createObject("component", "boxAPI.src.boxAPIHandler").init(  );
		return this;
	}

	public any function create(required string folderName, required string ParentID, required string asUserID) output=false {
	...
	public struct function copy(required string folderName, required string ParentID, required string SourceID, required string asUserID) output=false {
		local.jsonBody = structNew();
		local.jsonBody['name']   = arguments.folderName;
		local.jsonBody['parent'] = structNew();
		local.jsonBody['parent']['id'] = arguments.ParentID;
	...
	public any function getContents(required string folderID, required string asUserID) output=false {
		local.apiResponse = variables.boxAPIHandler.makeRequest(
			object     = this.objectName,
	...
	public any function getInfo(required string folderID, required string asUserID) output=false {
		local.apiResponse = variables.boxAPIHandler.makeRequest(
			object     = this.objectName,
			objectID   = arguments.folderID,
			httpMethod = "GET",
	...
	public any function update(required string folderID, string folderName, string parentID, array tags, required string asUserID) output=false {
		local.jsonBody = structNew();
	...
	public any function delete(required string folderID, required string asUserID) output=false {
		local.apiResponse = variables.boxAPIHandler.makeRequest(
			object      = this.objectName,
			objectID    = arguments.folderID,
	...
	public any function getCollaborations(required string folderID, required string asUserID) output=false {
		local.apiResponse = variables.boxAPIHandler.makeRequest(
			object     = this.objectName,
			objectID   = arguments.folderID,
